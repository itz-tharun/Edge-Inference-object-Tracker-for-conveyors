...
# NOTE: this is the FIXED-INTERVAL BASELINE. The drift/suspicion score is
# still computed and logged (for comparison purposes) but does NOT control
# triggering here - reclassification instead happens on a dumb fixed timer.
...
RECLASSIFY_INTERVAL = 30   # reclassify every N frames, regardless of drift signal
...
while True:
    ...
    for obj_idx, (contour, aspect_ratio, (x, y, w, h)) in matched_contours.items():
        ...
        if is_fully_in:
            ...
            should_trigger = never_classified or (total_frames % RECLASSIFY_INTERVAL == 0)
    ...
    cv2.imshow("Pen Detector - FIXED INTERVAL BASELINE", stacked)
...
import csv
with open("suspicion_log_fixed_interval.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["frame", "object_id", "suspicion_score", "triggered_reclassification"])
    writer.writerows(suspicion_log)
print("Suspicion log saved to suspicion_log_fixed_interval.csv")

with open("ground_truth_events_fixed_interval.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["frame", "note"])
    writer.writerows(ground_truth_log)
print(f"Ground truth events saved to ground_truth_events_fixed_interval.csv ({len(ground_truth_log)} events marked)")
