...
# --- MODEL WARM-UP ---
# The very first call to model.predict() pays a one-time cost building the
# TensorFlow computation graph, which can take several seconds. Do that here,
# before the timed test loop starts, so it doesn't contaminate your data.
...
# Instead of spawning a new threading.Thread for every classification attempt
# (which risks TensorFlow/Keras threading fragility - a model.predict() call
# from a freshly-spawned thread can occasionally hang, which is the most
# likely explanation if invocations mysteriously stall mid-test), ALL
# classification requests go through one queue, consumed by exactly one
# persistent worker thread that is the only thread that ever touches the
# model. This guarantees model.predict() is always called from the same
# thread context, every time.
inference_queue = queue.Queue()
pending_reclassification = defaultdict(bool)  # object_id -> is a request currently queued/running for it
...
DRIFT_THRESHOLD = 0.4        # suspicion score above this = "something may be wrong"
HYSTERESIS_FRAMES = 3        # must stay suspicious this many consecutive frames to trigger
W_APPEARANCE = 0.6           # weight on appearance drift in the suspicion score
W_TRACKING = 0.4             # weight on tracking/position drift in the suspicion score
...
suspicion_log = []                        # rows: frame, object_id, suspicion, appearance_drift, tracking_drift, requested, actual
...
def compute_suspicion_score(object_id, roi, actual_centroid):
    ...
    Returns (suspicion, appearance_drift, tracking_drift) - the components
    are returned separately so they can be logged individually, which makes
    it much easier to diagnose *why* a suspicion spike happened later.
    """
    # --- Appearance drift ---
    ...
    return suspicion, appearance_drift, tracking_drift
...
def inference_worker():
    """
    The ONLY function that ever calls model.predict(). Runs forever in one
    dedicated background thread, pulling classification requests off the
    queue one at a time. This guarantees the model is always called from
    the exact same thread context - if model.predict() were instead called
    from a freshly-spawned thread every time (as in the original design),
    occasional hangs are a known TensorFlow/Keras threading fragility risk,
    which is the most likely explanation for invocation counts mysteriously
    stalling mid-test.
    """
    while True:
        item = inference_queue.get()
        if item is None:  # sentinel to allow clean shutdown if ever needed
            break
        roi_frame, object_id = item
        try:
            ...
            print(f"[Object {object_id}] Inference done in {duration:.3f}s. Confidence: {conf:.4f}")

            # INVERTED LOGIC - Low confidence = PEN (matches original training setup)
            if conf < 0.5:
            ...
            pending_reclassification[object_id] = False


# Start the single worker thread once, before the video loop begins
...
while True:
    ...
    for obj_idx, (contour, aspect_ratio, (x, y, w, h)) in matched_contours.items():
        ...
        if is_fully_in:
            ...
            suspicion, app_drift, trk_drift = compute_suspicion_score(obj_idx, roi, centroid_now)
            ...
            should_trigger = never_classified or (consecutive_high_suspicion[obj_idx] >= HYSTERESIS_FRAMES)

            # requested_reclassification: did the drift logic WANT to reclassify?
            # actual_reclassification: did a request actually get queued this frame?
            # These can differ if a request is already pending for this object
            # (previous classification still running) - splitting them apart
            # makes it obvious in the log whether the mechanism is deciding
            # correctly vs. whether requests are getting stuck/backed up.
            requested_reclassification = should_trigger
            actual_reclassification = False

            if should_trigger and not pending_reclassification[obj_idx] and roi.size > 0:
                ...
                pending_reclassification[obj_idx] = True
                actual_reclassification = True
                inference_queue.put((roi.copy(), obj_idx))

            suspicion_log.append((
                total_frames, obj_idx,
                round(float(suspicion), 4),
                round(float(app_drift), 4),
                round(float(trk_drift), 4),
                requested_reclassification,
                actual_reclassification
            ))
    ...
    for obj_id in memory_ids_to_process:
        if obj_id not in detected_ids:
            ...
            if frame_since_last_seen[obj_id] <= MAX_FRAMES_WITHOUT_DETECTION:
                ...
                pending_reclassification.pop(obj_id, None)
                with results_lock:
...
suspicion_path = os.path.join(SCRIPT_DIR, "suspicion_log.csv")
with open(suspicion_path, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["frame", "object_id", "suspicion_score", "appearance_drift",
                      "tracking_drift", "requested_reclassification", "actual_reclassification"])
    writer.writerows(suspicion_log)
print(f"Suspicion log saved to {suspicion_path}")

ground_truth_path = os.path.join(SCRIPT_DIR, "ground_truth_events.csv")
with open(ground_truth_path, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["frame", "note"])
    writer.writerows(ground_truth_log)
print(f"Ground truth events saved to {ground_truth_path} ({len(ground_truth_log)} events marked)")
